...
public class SecurityConfig {
	...
	public JdbcUserDetailsManager users() {
		...
		ds.setUrl("jdbc:mysql://10.1.3.21:3306/springsecurity");
		ds.setUsername("root");
		ds.setPassword("root");
		//
		JdbcUserDetailsManager jdbc=new JdbcUserDetailsManager(ds);
		jdbc.setUsersByUsernameQuery("select user,pwd,enabled from users where user=?"); // autentica
		jdbc.setAuthoritiesByUsernameQuery("select user,rol from roles where user=?"); // autoriza
		return jdbc;
	} 
